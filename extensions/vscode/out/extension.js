...
function activate(context) {
    ...
    protocol = new protocol_1.RewindProtocol(port, ideName, vscode.version);
    recorder = new recorder_1.RewindRecorder(protocol);
    statusBar = new statusbar_1.RewindStatusBar();
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('rewind.toggleRecording', handleToggleRecording), vscode.commands.registerCommand('rewind.showStatus', handleShowStatus), vscode.commands.registerCommand('rewind.showActivity', handleShowActivity), vscode.commands.registerCommand('rewind.showProjects', handleShowProjects), vscode.commands.registerCommand('rewind.analyzeProject', handleAnalyzeProject), vscode.commands.registerCommand('rewind.searchHistory', handleSearchHistory), vscode.commands.registerCommand('rewind.importShellHistory', handleImportHistory), vscode.commands.registerCommand('rewind.openPermissions', handleOpenPermissions), statusBar, recorder);
    ...
    vscode.window.showInformationMessage(`Rewind IDE active — recording ${ideName} activity to local SQLite database`);
}
function deactivate() {
    if (recorder) {
        recorder.stop();
...
async function handleShowStatus() {
    try {
        ...
        vscode.window.showWarningMessage('Rewind server not running. Start with: rewind ide start');
    }
}
